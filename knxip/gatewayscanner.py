...
class GatewayScanner:
    ...
    def __init__(self, broadcast_source_ip_address="0.0.0.0",
        ...
        :param broadcast_source_ip_address: IPv4 Address where we send the Search Request out and listen for response
    ...
    def start_search(self):
        ...
        coroutine_broadcaster = self._asyncio_loop.create_datagram_endpoint(
            lambda: self.KNXSearchBroadcastProtocol(self._asyncio_loop,
                                                    self._listener_transport.get_extra_info('sockname')[1]),
        ...
        if self._resolved_gateway_ip_address is None and self._resolved_gateway_ip_port is None:
            ...
            logger.debug("Gateway found: {}:{}".format(self._resolved_gateway_ip_address,
    ...
    def _timeout_handling(self):
        ...
        logger.error("Listener don't receive any packets, timeout occur(Timeout in Seconds: {} !",
        ...
        def connection_made(self, transport):
            ...
            self.transport.sendto(
                self._build_search_request_data(
                    self.transport.get_extra_info('sockname')[0], self._listener_port
                )
            )
