...
class KNXMessage(object):
    ...
    def to_frame(self):
        self.sanitize()
        res = []
        res.append((1 << 7) + (1 << 4) + (self.repeat << 5) + (self.priority << 2))
    ...
    def from_frame(cls, frame):
        ...
        if (checksum % 0x100) != frame[len(frame)-1]:
            raise KNXException('Checksum error in frame {}, expected {}, but got {}'.format(tohex(frame),frame[len(frame)-1],checksum % 0x100))
